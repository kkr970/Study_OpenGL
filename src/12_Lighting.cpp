...
int main(){
    ...
    {   
        ...
        processInput(window);
        //spotlight 켜고 끄기
        if(glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS && !isFkeyPressed && !isOnSpotLight)
        {
            isFkeyPressed = true;
        }
        if(glfwGetKey(window, GLFW_KEY_F) == GLFW_RELEASE && isFkeyPressed && !isOnSpotLight)
        {
            isOnSpotLight = true;
            isFkeyPressed = false;
        }
        ...
        // spotLight
        lightingShader.setBool("isOnSpotLight", isOnSpotLight);
